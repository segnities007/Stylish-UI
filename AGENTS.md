...
UI コードは**意味単位**で関数に分割すること。行数は基準ではない。

- **原則: 1関数 = 1つの意味的責務。**
  読み手がコードのブロックを見て「これは何をしている？」と一瞬でも考えたら、
  そのブロックは**名前を持つ関数**であるべき。良い状態 = 関数名を読むだけで
  画面の構成（目次）が分かること。
- 判断基準（行数ではなく意味で判定する）:
  - 「ヘッダー」「FAB」「一覧」「カード」「ダイアログ」…と言い換えできる塊は関数に切る
  - 空行やコメントで区切られた区間が 2 つ以上ある関数は、ほぼ確実に分割対象
  - 分岐（if/when）の各枝が意味単位なら、枝ごとに関数化する
  - 入力状態が 3 つ以上並ぶなら `XxxState` ホルダー + `rememberXxxState()` に集約
- **画面 Composable = オーケストレーター**: 状態 + コールバック + パーツ組み立てのみ。
  描画ロジック・分岐・長いモディファイア列を直接書かない。
...
`scripts/verify-composable-size.py`（80行超で `check`/`preBuild` が失敗）は、
意味分割を怠った結果を検知するための背後のセーフティネット。
ベースライン・ラチェット方式（既存違反は `scripts/composable-size-baseline.txt` 登録済み、
**新規違反のみ失敗**）。減らすのは自由、増やすのは禁止、しきい値緩和や
`size:allow` の乱用は禁止。ただし 80 行未満でも意味単位に分割すること。

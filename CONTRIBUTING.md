...
## 品質監査（docs/quality-audit.md）の使い方

[docs/quality-audit.md](docs/quality-audit.md) は、Stylish UI の欠点を
ID 付きで列挙した作業ファイルです。作業の割り振りに使用してください。

1. **作業を選ぶ**: 各項目に `pending` / `in-progress` / `done` のステータスがあります。
   `pending` の項目から、自分のワークストリーム（WS）のものを選びます。
2. **作業開始時**: 対象項目のステータスを `in-progress` に更新します。
3. **実装完了時**: 項目のステータスを `done` に更新します。対応方針の欄が
   「要判断」のままなら、判断結果と決定を「判断結果」テーブルに記録してください。
4. **まとめて更新する**: 1 つの PR で複数の項目を完了した場合は、1 つのコミットで
   まとめて更新して構いません。
5. **対象外の判断**: 対応しないと判断した項目は `done` ではなく、対象外の理由を
   明記した上でステータスを更新してください。

...
1. 公式サイトまたは `:website-wasm:wasmJsBrowserRun` を起動する。
2. カテゴリタブで対象領域を絞り込む。
...
Wasm ブラウザでは `Ctrl/Cmd+F` の代わりにカタログ内検索を使うと、表示中のデモだけを
対象にできます。暗色/明色の切替はヘッダーのテーマボタンで行います。Reduced motion が
有効な環境では、カテゴリ切替などの遷移を即時化します。
